...
typedef struct {
    ...
    int64_t available_send_kbitrate; /* 可用发送码率(kb) */
    int64_t target_encode_kbitrate;  /* 目标编码码率(kb) */
    int64_t actual_encode_kbitrate;  /* 实际编码码率(kb) */
    int64_t retransmit_kbitrate;     /* 重传码率(kb) */
...
@property (nonatomic, assign) uint32_t capture_fps;   // 采集帧率
@property (nonatomic, assign) uint32_t input_fps;     // 输入编码帧率
@property (nonatomic, assign) uint32_t encode_fps;    // 编码帧率
@property (nonatomic, assign) uint32_t sent_fps;      // 发送帧率
@property (nonatomic, assign) uint32_t averge_qp;     // 平均qp
@end
...
@property (nonatomic, copy, readonly) NSDictionary<NSString*,AliRtcAuthInfo*> *destChannelInfos;

- (void)addDestChannelInfoWithChannelId:(NSString *)channeId
                               authInfo:(AliRtcAuthInfo *)authInfo;

- (void)removeDestChannelInfo:(NSString *)channeId;
 ...
 * @param code 错误码
 */
- (void)onChannelRelayStateChanged:(int)state code:(int)code;
 ...
 * @return 0 Success 非0 Failure
 */
- (int)startChannelRelay:(AliRtcChannelRelayConfiguration *)configuration;
 ...
 * @return 0 Success 非0 Failure
 */
- (int)updateChannelRelay:(AliRtcChannelRelayConfiguration *)configuration;
